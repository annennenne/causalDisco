...
This document tracks Tetrad algorithms with respect to their status in this
project: whether they are already documented, should be implemented, or should
be removed. Notes include limitations such as support for background knowledge,
score/test requirements, and nonstandard outputs.
...
- `ccd` (test-based) Produces a **cyclic PAG**, where edge semantics differ from
  standard PAGs. Tetrad says no background knowledge, but seems only required
...
- `DAGMA` (neither score- nor test-based) Does not accept background knowledge.

- `Direct-LiNGAM` (score-based) Does not accept background knowledge.

- `ica_lingam` (neither score- nor test-based) Does not accept background
  knowledge.
