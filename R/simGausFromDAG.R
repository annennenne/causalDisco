...
simGausFromDAG <- function(amat, n, regparLim = c(0.5, 2),
  ...
  residualsds <- stats::runif(p, min = resSDLim[1], max = resSDLim[2])

  # first col (always exogenous)
  data[, 1] <- stats::rnorm(n, sd = residualsds[1])


  for (i in 2:p) {
    # regression parameters
    pars <- stats::runif(p, min = regparLim[1], max = regparLim[2]) *
      sample(c(-1, 1), p, replace = TRUE, prob = c(pnegRegpar, 1 - pnegRegpar))
    usepars <- pars * amat[i, ]

    thisVar <- data %*% usepars + stats::rnorm(n, sd = residualsds[i])

    if (standardize) thisVar <- (thisVar - mean(thisVar)) / stats::sd(thisVar)
