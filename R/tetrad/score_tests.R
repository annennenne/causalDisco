...
jar_command <- sprintf(
  "jar tf %s | grep edu/cmu/tetrad/algcomparison/score",
  jar_path
)
...
test_that("use_mixed_variable_polynomial sets correct score", {
  ts_obj$use_mixed_variable_polynomial(
    structure_prior = 0,
    f_degree = 2,
    discretize = TRUE
  )
...
test_that("use_conditional_gaussian_score sets correct score", {
  ts_obj$use_conditional_gaussian_score(
    penalty_discount = 1,
    discretize = TRUE,
...
test_that("use_degenerate_gaussian_score sets correct score", {
  ts_obj$use_degenerate_gaussian_score(
    penalty_discount = 1,
    structure_prior = 0
  )
  expect_true(!is.null(ts_obj$score))
