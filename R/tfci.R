...
tfci <- function(data = NULL, order, sparsity = 10^(-1), test = regTest,
  ...
  # note: twd is handled by the test: they have this as default, so the code
  # here is used to ensure that missing info is only passed along if we in fact
  # want to use twd
  if (any(is.na(data))) {
    if (methodNA == "none") {
  ...
  # make testing procedure that does not condition on the future
  ...
  fci_skel <- pdsep(
    skel = skel, suffStat = thisSuffStat,
    indepTest = thisDirTest,
    p = nvar,
    sepset = skel@sepset,
    pMax = skel@pMax,
  ...
  # pack up output
  out <- list(
    tamat = tamat(amat = t(res), order = order, type = "ag"),
    psi = sparsity
  )
  class(out) <- "tpag"
