...
- Is required/forbidden directional? E.g. is `required(A ~ B)` the same as `required(B ~ A)`?

Documentation at `?knowledge` seems to suggest it is directional, but it doesn't seem to be implemented that way?
...
And it only tries to fix it form the other way? I.e. if to and from were swapped? More investigation needed ...
