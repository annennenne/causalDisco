...
- In documentation of test / scores say which data types they support (continuous, discrete, mixed). It's written for
some of the engines, but even that is unclear sometimes.
...
- We are mixing between different things currently (since we rely on `caugi` are it uses `data.frame` and `S7`):
  - `tibble` vs `data.frame` (e.g. `knowledge` is `tibble` and `disco()$caugi@edges` is `data.frame`).
    - I added a `nodes()` and `edges()` method to `knowledgeable_caugi` objects to easily return as `tibble`.
