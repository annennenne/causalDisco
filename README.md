...
# We can also pass background knowledge to the algorithms that support it. Here we use tiered knowledge,
# which is a common way to encode temporal ordering of variables. You can use tidyselect helpers to specify which
# variables belong to which tier.
kn <- knowledge(
  tpc_example,
  tier(
...
bnlearn_pc <- pc(
  ...
  test = "cor", # Use correlation test for conditional independence
  alpha = 0.05 # Significance level for the test
)
