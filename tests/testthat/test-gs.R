test_that("gs bnlearn disco respects tier knowledge", {
  data(tpc_example)
  ...
  bnlearn_gs <- gs(engine = "bnlearn", test = "mi_g")
  output <- disco(
    data = tpc_example,
    method = bnlearn_gs,
    knowledge = kn
  ...
  bnlearn_gs <- gs(engine = "bnlearn", test = "mi_g")
  output <- disco(tpc_example, bnlearn_gs, knowledge = kn)
  edges <- output$caugi@edges
...
test_that("gs bnlearn disco respects required background knowledge", {
  data(tpc_example)
...
test_that("gs bnlearn disco respects forbidden background knowledge", {
  data(tpc_example)
...
test_that("gs bnlearn disco works with additional alg args", {
  data(num_data)
  gs_bnlearn <- gs(
    engine = "bnlearn",
    test = "mi_g",
  ...
  out <- disco(num_data, gs_bnlearn)

  expect_equal(class(out), c("knowledgeable_caugi", "knowledge"))
})

test_that("gs bnlearn disco works with additional test args", {
  data(num_data)
  gs_bnlearn <- gs(
    engine = "bnlearn",
    test = "mc_zf",
