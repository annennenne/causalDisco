...
rdata_to_tetrad <- function(df, int_as_cont = FALSE) {
  ...
  for (col in 1:p) {
    for (row in 1:n) {
      ...
      .jcall(databox, "V", "set", as.integer(row - 1), as.integer(col - 1), valueObj)
    }
  }
...
tetrad_data_to_rdata <- function(data) {
  ...
  for(i in 0:(numVars - 1)) {
    varNames[i+1] <- as.character(.jcall(namesList, "Ljava/lang/Object;", "get", as.integer(i))$toString())
  }
  ...
  for(row in 0:(n - 1)) {
    for(col in 0:(numVars - 1)) {
      df[row+1, col+1] <- .jcall(data, "Ljava/lang/Object;", "getObject", as.integer(row), as.integer(col))
    }
  }
  return(df)
}
...
graph_to_matrix <- function(g, nullEpt = 0, circleEpt = 1, arrowEpt = 2, tailEpt = 3) {
  endpoint_map <- list("NULL" = nullEpt,
                       "CIRCLE" = circleEpt,
                       "ARROW" = arrowEpt,
                       "TAIL" = tailEpt)
  ...
  for(i in 0:(numEdges - 1)) {
    ...
    ep1 <- as.character(.jcall(edge, "Ljava/lang/Object;", "getEndpoint1")$name())
    ep2 <- as.character(.jcall(edge, "Ljava/lang/Object;", "getEndpoint2")$name())
  ...
  for(i in 0:(p - 1)) {
    ...
    colNames[i+1] <- as.character(.jcall(node, "Ljava/lang/Object;", "toString"))
  }
...
tetrad_matrix_to_matrix <- function(array) {
  ...
  for(i in 0:(nrows - 1)) {
    for(j in 0:(ncols - 1)) {
      mat[i+1, j+1] <- .jcall(array, "D", "get", as.integer(i), as.integer(j))
    }
  }
  return(mat)
}
...
tetrad_matrix_to_rdata <- function(array, variables) {
  ...
  for(i in 0:(ncols - 1)) {
    cols[i+1] <- as.character(.jcall(variables, "Ljava/lang/Object;", "get", as.integer(i))$toString())
  }
...
write_gdot <- function(g, gdot) {
  ...
  for(i in 0:(numEdges - 1)) {
    edge <- .jcall(edges, "Ljava/lang/Object;", "get", as.integer(i))
    node1 <- as.character(.jcall(.jcall(edge, "Ljava/lang/Object;", "getNode1"), "Ljava/lang/Object;", "getName"))
    node2 <- as.character(.jcall(.jcall(edge, "Ljava/lang/Object;", "getNode2"), "Ljava/lang/Object;", "getName"))
    endpoint1 <- as.character(.jcall(.jcall(edge, "Ljava/lang/Object;", "getEndpoint1"), "Ljava/lang/Object;", "name"))
    endpoint2 <- as.character(.jcall(.jcall(edge, "Ljava/lang/Object;", "getEndpoint2"), "Ljava/lang/Object;", "name"))
