...
The example discussed in this section is inspired by the Julia package [CausalDiscovery.jl](https://mschauer.github.io/CausalInference.jl/latest/)
with their PC algorithm example, which can be found here [PC algorithm example in CausalDiscovery.jl](https://mschauer.github.io/CausalInference.jl/latest/examples/pc_basic_examples/).
...
`x` and `w` has no arrows at either end, showing that the direction of causal influence cannot be determined from the data alone. Both directions; `x` to `w` and `w` to `x`,
