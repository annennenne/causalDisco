...
To encode this knowledge, you need to create a `knowledge` object using the `knowledge()` function. The first argument
is (optionally, but very recommended to ensure names match) the dataset, and then you can specify tiered knowledge using
the `tier()` function. Here we create a tiered knowledge structure based on the life stages:
