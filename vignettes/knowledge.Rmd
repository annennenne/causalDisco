...
We will use the `tpc_example` dataset from the causalDisco package for the
following examples:
...
The causal discovery algorithms respects the provided knowledge. We can plot the
...
All knowledge types are supported with bnlearn engine. When required knowledge
...
causalDisco engine only supports tiered and forbidden knowledge. If required
knowledge is provided, it will give a warning and ignore the required knowledge.

## pcalg

Only forbidden symmetric knowledge is supported for pcalg engine. That is, edges
that are forbidden in both directions. Thus, the only type of knowledge that can
be used with pcalg is knowledge created using forbidden edges (`%!-->%`) without
any required or tier knowledge. An example is shown below:
