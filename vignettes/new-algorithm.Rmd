...
Both bnlearn and pcalg are R packages and therefore share the same interface, while Tetrad is a Java application and
requires a different integration pattern.

# bnlearn and pcalg

Suppose we want to add the Hybrid Parents and Children (HPC) algorithm from bnlearn. We can add it to
...
The function takes an engine argument, which specifies which implementation of the algorithm to use. Even though HPC
is only available via bnlearn here, using an engine argument allows the interface to remain consistent with other
algorithms that may have multiple backends. The `...` argument allows us to pass additional arguments to the underlying
algorithm and test. 

The code for the `hpc()` function then looks as follows:
...
The builder function calls `hpc_bnlearn_runner()` (which we haven't yet defined!), which is responsible for running the
algorithm on the data.
This function receives the test, the significance level, and any additional arguments passed via `...`.

It creates a new `BnlearnSearch` object, configures the test and algorithm, and returns a *runner*: a list containing
two functions, `set_knowledge()` and `run()`. The `set_knowledge()` function applies background knowledge to the search
object (constructed from `knowledge()`), while `run()` executes the algorithm on a data set.
...
`hpc()`, and then pass it to `disco()`. We use the `tpc_example` dataset included with causalDisco for demonstration
purposes.
...
Tetrad algorithms require a different integration pattern, since Tetrad is a Java application rather than an R package.
To add a new Tetrad algorithm, we first register it using `register_tetrad_algorithm()`. This function takes
the name of the algorithm and a setup function that configures a `TetradSearch` object to run the algorithm.
The setup function receives the `TetradSearch` object as its first argument, and any additional arguments passed via
`...` when constructing the method. It is responsible for configuring the `TetradSearch` object with the appropriate
parameters and score, and for instantiating the correct Tetrad algorithm class. The class path can, for example, be
found by looking at the Tetrad source code.
...
We define the runner function `my_boss_variant_tetrad_runner()`, where we pass additional arguments using the splice
operator `!!!` from rlang.
