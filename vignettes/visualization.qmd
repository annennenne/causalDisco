...
1. **Builder function:** Constructs a *runner* for the algorithm. It configures
   the algorithm with the provided parameters and optional background knowledge.

2. **Method object:** Created via `new_disco_method()`. It encapsulates the
   builder and metadata about the method. This allows us to be able to call
   `disco()` to execute the algorithm on a dataset.

Next, we define the runner. The runner handles configuring the search object
with the chosen test, alpha level, and any additional arguments, and it actually
runs the search on the data when called. For bnlearn algorithms, we use the
`BnlearnSearch` R6 class to manage search configuration and execution. We also
use `distribute_engine_args()` to determine which extra arguments should be
passed to the test, the algorithm, or both.
...
rather than `test` and `alpha`, and the runner would call `search$set_score()`
instead of `search$set_test()`. A **hybrid** algorithm algorithm would accept
`test`, `alpha`, and `score` arguments, and the runner would call both
`set_test()` and `set_score()`.
...
Like for bnlearn and pcalg, we then create a method function `my_boss_variant()`
that constructs a builder function which in turn constructs a runner using the
registered Tetrad algorithm.

BOSS is a **score-based** algorithm, so the method accepts a `score` argument
and passes it to the runner, which configures the `TetradSearch` object
accordingly. It returns a PDAG, so we set the `graph_class` attribute to
`"PDAG"`.

```{r}
#| label: my-boss-variant
